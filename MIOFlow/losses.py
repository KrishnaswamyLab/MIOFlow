...
# %% auto 0
__all__ = ['MMD_loss', 'OT_loss', 'density_specified_OT_loss', 'Density_loss', 'Local_density_loss', 'EnergyLossSeq',
           'EnergyLossGrowthRateSeq', 'EnergyLoss', 'EnergyLossGrowthRate']
...
class density_specified_OT_loss(nn.Module):
    ...
    def __init__(self, which='sinkhorn', reg=0.1, reg_m=1.0, take_softmax=True, detach_M_ot=False):
        if which not in self._valid:
            raise ValueError(f'{which} not known ({self._valid})')
        elif which == 'sinkhorn':
...
class Density_loss(nn.Module):
    ...
    def __call__(self, source, target, groups = None, to_ignore = None, top_k = 5, pre_softmax_weights=None):
        if pre_softmax_weights is not None:
            post_softmax_weights = torch.nn.functional.softmax(pre_softmax_weights, dim=-1) * pre_softmax_weights.shape[-1]
        if groups is not None:
            # for global loss
            if pre_softmax_weights is not None:
                c_dist = torch.stack([
                    torch.cdist(source[i], target[i]) * post_softmax_weights[i]
            ...
            if pre_softmax_weights is not None:
                c_dist = torch.stack([
                    torch.cdist(source, target) * post_softmax_weights
                ])
            else:
                c_dist = torch.stack([
...
class EnergyLossGrowthRateSeq(nn.Module):
    def __init__(self, weighted=True, detach_m=False):
    ...
    def __call__(self, model, xtseq, mtseq, t):
        ...
        if self.weighted:
            masses = torch.nn.functional.softmax(mtseq, dim=-1) * mtseq.shape[-1]
            if self.detach_m:
...
class EnergyLossGrowthRate(nn.Module):
    ...
    def __call__(self, model, x, m, t):
        ...
        if self.weighted:
            masses = torch.nn.functional.softmax(m, dim=-1) * m.shape[-1]
            if self.detach_m:
                masses = masses.detach()
            dxdt = dxdt * masses.unsqueeze(-1) # weighted by mass
        return torch.square(dxdt[...,:-1]).mean(), torch.square(dxdt[...,-1]).mean()
